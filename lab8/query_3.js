...
function _buildExpressionAttributes(
...
<FMI>){
    ...
    function _buildFilterExpression(dragon_id_arr){
        ...
        return filter_str.<FMI>(0, -2) + ")"; // remove , and space and add close
        }
        ...
        var dragons_arr = <FMI>(data.Items);
        cb(null, dragons_arr);
        });
        }
        ...
        stageTwo(dragon_id_arr, <FMI>(err, dragon_arr){
        if(err){
