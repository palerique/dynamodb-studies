...
exports.handler = function (event, context, callback) {
    ...
    if (event["user_name_str"] === undefined || event["session_id_str"] === undefined) {
        ...
        confirmAdminLogin(event["user_name_str"], event["session_id_str"], function (err, success_boo) {
            ...
            const update_str = constructUpdate(event.updates),
                expression = constructExpressionObject(event.updates);

            console.log(update_str, expression, event.updates);
...
const BUCKET_STR = "paulo-lerbach",
    AWS = require("aws-sdk"),
...
//would have kicked in before this
...
function getUpdatedItemFromJson(dragon_name_to_edit_str) {
    const file_path_str = "/Users/ph/area-de-trabalho/POCs/dynamodb-poc/lab7/resources/",
...
async function handleSpecialCase(original_dragon_name_str, updated_attributes, cb) {
    console.log("special case we need a transaction here", original_dragon_name_str, updated_attributes);
    const query_response = await getOldDragonItem(original_dragon_name_str);
    if (query_response.Items.length === 0) {
        return cb("no dragon found called " + original_dragon_name_str, null);
    }
    const dragon = query_response.Items[0];
    console.log('Original entity on table: ', dragon);
    dragon.dragon_name = {
        S: updated_attributes.dragon_name_str
    };
    if (updated_attributes.damage_int
        && updated_attributes.damage_int > 0
        && updated_attributes.damage_int < 11) {
        dragon.damage = {
    ...
    if (updated_attributes.protection_int
        && updated_attributes.protection_int > 0
        && updated_attributes.protection_int < 11) {
        dragon.protection = {
...
async function runTransaction(dragon, original_dragon_name_str, callback) {
    ...
    console.log('Running transaction (Delete + Put): ', params);
...
if (process.argv[2] === "test") {
    confirmAdminLogin(process.argv[3], process.argv[4], function (err, success_boo) {
        ...
        const updated_attributes = getUpdatedItemFromJson(process.argv[5]),
        ...
        if (updated_attributes.dragon_name_str) {
            ...
            const update_str = constructUpdate(updated_attributes),
